        ...
        {#each posts as post (post.id)}
          <div class="col-lg-3 col-md-4 col-12">
            <div class="card p-2 m-2 shadow" on:click={() => navigateToPlayer(post.file_code)}>
              ...
              <p style="font-weight: bold; font-size: 13px">ditonton : {post.views}</p>
              <p style="font-weight: bold; font-size: 13px">Diupload : {post.uploaded}</p>
            </div>
          </div>
        {/each}
      </div>
