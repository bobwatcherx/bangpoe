 ...
 const apiurl = 'https://voe.sx/api/file/list?key=Wr7fjmWTBp6EY0XGYJZwleaMJiJ2cuf21c3UvSpDd7GtPLAVnQTGiY9RNtwCyCbK';
  ...
  console.log(id)
      ...
      async function fetchRelatedVideos() {
          ...
          console.log("dq",data)
          const totalPages = data.result.total;
          console.log("total ",totalPages)
          const randomPage = Math.floor(Math.random() * totalPages);
          console.log(randomPage)
          const responseRandomPage = await fetch(`${apiurl}&page=1&per_page=25`);
          const dataRandomPage = await responseRandomPage.json();
          console.log(dataRandomPage)
...
<style >
  ...
  {:else}
    <div class="mt-3">
      <iframe src={`https://voe.sx/e/${id}`} width="100%" height="300px" frameborder="0" allowfullscreen allowscrolling="no"></iframe>
